        ...
        while (quantumRemaining > 0) {
            ...
            if(!cpuIdle) { // Se a cpu não está em espera, ocorre tudo normalmente
                ...
                if(currentProcess.isActive == FALSE) { // Processo finalizado
                    ...
                    quantumUsed = 0;
                    finishedProcesses += 1;
                    cpuIdle = TRUE; // habilita a Ociosidade da cpu aguardando um novo quantum
                }
                // Checagem de ciclos de I/O
                    ...
                    logPrintf(log, "Processo ID: %d | Situação: Paralisado | Enviado -> fila de I/O\n", currentProcess.id);
                    quantumUsed = 0;
                ...
                logPrintf(log, "CPU em espera, aguardando até o proximo Quantum.\n");
                logPrintf(log, "--------------------------------------------------------------\n");
