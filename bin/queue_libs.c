    ...
    if(!isEmpty(queue)) {
        if(details == TRUE) {
            ...
            for(int i=0; i < queue->size; i++) { // Percorre a fila inteira exibindo cada processo presente.
                ...
                logPrintf(log, "Nome: %s | ID: %d | Ciclos totais: %d | Ciclos restantes: %d | Ativo: %d | Entrada em %d ciclos\n", 
                curr.name, curr.id, curr.totalCycles, curr.remainingCycles, curr.isActive, curr.arrivalCycles);
                logPrintf(log, "Prioridade: %d | After cycles: %d | type: %d | Ativo: %d | Requerido: %d | Ciclos I/O: %d\n",
